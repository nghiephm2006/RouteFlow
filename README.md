...
- **Tự động Tìm Kiếm (Autocomplete)**: Nhập liệu tên đường tiện lợi với API Nominatim, tự fix số nhà. FlyTo (bay) bản đồ đến vị trí rất mượt.
...
Hệ thống API RESTful đóng vai trò cung cấp, quản lý dữ liệu Master (Orders) và trong tương lai hỗ trợ các tính toán phức tạp (như phân cụm - Clustering).
...
- Tích hợp tính năng gọi API Backend để hiển thị/thêm bớt "Order" (Đơn Hàng) từ Frontend.
- Import danh sách đơn hàng từ tệp **Excel (.xlsx)** bằng EPPlus ở giao diện Web.
- Giao việc tự động: Chia nhóm đơn hàng (Clustering) các điểm giao gần nhau cho các tài xế (Shipper) khác nhau để tối đa hoá hiệu suất.
